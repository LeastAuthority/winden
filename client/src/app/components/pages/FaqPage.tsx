...
<p>
  This means that you
  <em> <b>both</b> need to be online at the same time </em> for the transfer
  to complete.
</p>
<p>
  It also means that <em>we never store your files on our servers</em>. The
  file stays on your device, and only passes through our servers when the
  recipient is downloading the file. As files are end-to-end encrypted we
  cannot read them, and since they are never stored on our servers you
...
<p>
  It means that we are still making many improvements and that the tool may
  experience unexpected issues. It also means that we are hoping to use this
  moment to learn from &lsquo;real people&rsquo; (like you) trying out the
  tool and sharing your experience with us.
</p>
<p>Some known limitations:</p>
<ul>
  <li>
    Single files: It is currently only possible to send one file at a time.
  </li>
  <li>
    File size limits: In this initial stage, it is only possible to send
    files of up to 200 MB. When sending files to mobile devices, this number
    may be smaller than 100 MB.
  </li>
  <li>
  ...
  <li>
    Error messages: We are not always able to determine the exact reason for
    an error occurring and therefore not able to provide the exact
  ...
  <li>
    Browsers: Firefox on iOS and Safari on iOS (before
...
<p>
  ...
  technology (see
  <a href="/about">About</a>). We are currently exploring the best-suited privacy-respecting business
  model. We are offering consulting and customization services for
  businesses, and in the future we may offer premium apps, features, or
  performance at extra cost, aside from the freely accessible version.
</p>
...
<ol>
  ...
  <li>
    ...
    This link contains the one-time code (for example, 7-guitarist-revenge)
    needed to receive the file.
    <br />
    <br />
    For ease of use you can copy the link using the Copy button. Pass on the
    link/code to the intended recipient verbally, digitally, or by any other
    means that you deem safe. You can either give the entire link or just
    the code and the website address (winden.app) to the recipient, so they
    can receive the file. Note that anyone with access to the link can
    receive your file.
  </li>
  <br />
  <li>
    <b>
      When the recipient opens the link and agrees to download the file, the
      transfer begins from your device
    </b>
    <br />
...
<ol>
  ...
  <li>
    ...
    <br />
    If you are sure you would like to receive this file, press Download. It
    will be downloaded to your browser memory. If prompted at the end, you
    may need to click &lsquo;Save file&rsquo; or similar in your browser to
    write it to your device&rsquo;s memory.
  </li>
  <br />
  <li>
...
<p>
  ...
  65,536 chance to guess right, which is a very small chance. If they guess
  wrong, the transfer is invalidated. This is tough luck for the sender, but
  also a safety feature: It prevents people from being able to keep guessing
  until they find the correct word combination.
</p>
...
<p>
  ...
  code is cleared for new usage. If the code would stay &lsquo;alive&rsquo;
  for other recipients, it would lose the security of being hard to guess.
</p>
...
<ul>
  ...
  <li>
    ...
    only the sender and recipient can read them. We cannot access the files.
  </li>
</ul>
...
<ul>
  <li>
    ...
    using Diffie&ndash;Hellman key exchange algorithm to establish a shared
    "mailbox" between two parties.&nbsp;
  </li>
  <li>
    Encryption via the relay servers use Salsa20 stream cipher and Poly1305
    MAC algorithm via the NaCl Secretbox abstraction.
  </li>
</ul>
...
<p>
  ...
  minimizing personal data that we collect. Specifically, we are not asking
  for and therefore do not know contact information (name, email address,
  phone number) about people who send/receive files. We do not store and
  cannot read files that are sent using Winden.
</p>

<p>
  Nevertheless, third parties such as our OVHcloud hosting provider collect
  some user access information like users&rsquo; IP address and user-agent.
  We have no control of that and seek no access to such information.
</p>
...
<p>
  ...
  <em>guarantee</em> that the code that we are making available is the same
  code that you receive in your browser, as it could be manipulated by
  others on the way. That&rsquo;s not us, this is any website or web-based
  application that you use. If you prefer code that you can verify and only
  need to <em>trust on first use</em>, you can rely on secure and open
  source downloadable applications, such as
...
<p>
  ...
  malicious, download the files through the link/code you copied). Here too,
  it is good to know that this doesn&rsquo;t just apply to Winden, but to
  any piece of information you have on your clipboard in general.
</p>
...
<p>
  ...
  way to transfer files safely from one device to another. Winden implements
  a Magic Wormhole application for the web.
</p>
...
<p>
  ...
  <a href="mailto:contact@winden.app">contact@winden.app</a> and we&rsquo;ll
  do our best to help!
</p>
`;
